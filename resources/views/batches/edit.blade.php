...
@section('actions')
    <a class="btn primary" href="{{ route('batches.print', $batch) }}" target="_blank"><i class="fa-solid fa-print"></i>Cetak</a>
                                ...
                                @foreach ($group['items'] as $item)
                                    <tr data-item-row="{{ $item->id }}">
                                        <td style="min-width:118px;width:118px">
                                            <input type="date" name="items[{{ $item->id }}][transaction_date]" value="{{ old('items.'.$item->id.'.transaction_date', optional($item->transaction_date)->format('Y-m-d')) }}">
                                        </td>
                                        <td style="min-width:104px;width:104px">
                                            <input name="items[{{ $item->id }}][proof_number]" value="{{ old('items.'.$item->id.'.proof_number', $item->proof_number) }}" required>
                                        </td>
                                        @if ($loop->first)
                                            <td rowspan="{{ $group['count'] }}" style="min-width:220px;width:220px;background:#172238">
                                                <div class="grid" style="gap:8px">
                                                    <span class="badge">{{ $group['proof'] }} / {{ $group['count'] }} baris</span>
                                                    <span class="text-xs font-bold text-slate-400">Rp. {{ number_format($group['total'], 0, ',', '.') }}</span>
                                                    <input name="receivers[{{ $group['proof'] }}]" value="{{ old('receivers.'.$group['proof'], $group['receiver']) }}" placeholder="Nama penerima">
                                                </div>
                                            </td>
                                        @endif
                                        <td style="min-width:104px;width:104px">
                                            <input name="items[{{ $item->id }}][activity_code]" value="{{ old('items.'.$item->id.'.activity_code', $item->activity_code) }}">
                                        </td>
                                        <td style="min-width:142px;width:142px">
                                            <input name="items[{{ $item->id }}][account_code]" value="{{ old('items.'.$item->id.'.account_code', $item->account_code) }}">
                                        </td>
                                        <td style="min-width:72px;width:72px">
                                            <input name="items[{{ $item->id }}][detail_code]" value="{{ old('items.'.$item->id.'.detail_code', $item->detail_code) }}">
                                        </td>
                                        ...
                                        <td style="min-width:116px;width:116px">
                                            <input type="number" name="items[{{ $item->id }}][amount]" value="{{ old('items.'.$item->id.'.amount', $item->amount) }}" required>
                                        </td>
                                        <td style="min-width:94px;width:94px">
                                            <button class="btn danger" type="button" data-delete-item="{{ $item->id }}"><i class="fa-solid fa-trash"></i>Hapus</button>
                                        </td>
                                    </tr>
                                @endforeach
                            @endforeach
